...
// 管理者: 単一予約取得
reservations.get("/detail/:id", async (c) => {
...
reservations.post("/create", async (c) => {
    ...
    // 一意な64文字トークン生成
    const token = Array.from({ length: 64 }, () => "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789".charAt(Math.floor(Math.random() * 62))).join("");
...
// トークンベース: キャンセルページ取得
reservations.get("/cancel/:token", async (c) => {
...
// トークンベース: 返却ページ取得
reservations.get("/return/:token", async (c) => {
    const token = c.req.param("token");
    const reservation = await prisma.reservation.findFirst({ where: { token } });
    if (!reservation) {
        return c.json({ error: "Not found" }, 404);
...
// 管理者: 予約承認
reservations.put("/detail/:id/approve", async (c) => {
    const id = Number(c.req.param("id"));
    // TODO: 認可
...
// 管理者: 予約完了
reservations.put("/detail/:id/complete", async (c) => {
    const id = Number(c.req.param("id"));
    // TODO: 認可
    const updated = await prisma.reservation.update({
        where: { id },
...
// 管理者: 予約キャンセル
reservations.put("/detail/:id/cancel", async (c) => {
    const id = Number(c.req.param("id"));
    // TODO: 認可
    const updated = await prisma.reservation.update({
        where: { id },
...
// 管理者: 予約却下
reservations.put("/detail/:id/reject", async (c) => {
    const id = Number(c.req.param("id"));
    // TODO: 認可
    const updated = await prisma.reservation.update({
        where: { id },
...
// 管理者: 予約返却
reservations.put("/detail/:id/return", async (c) => {
    const id = Number(c.req.param("id"));
    // TODO: 画像アップロード・認可
    const updated = await prisma.reservation.update({
        where: { id },
...
// 管理者: 予約削除
reservations.delete("/detail/:id", async (c) => {
    const id = Number(c.req.param("id"));
    // TODO: 認可・画像削除
